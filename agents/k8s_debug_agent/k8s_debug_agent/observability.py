...
def _patch_openai_client_for_context() -> None:
    """
    Patch OpenAI to use a global context store.

    Since asyncio context doesn't propagate correctly through AutoGen's
    internal async machinery, we use a module-level context store that
    gets updated when our instrumented methods run and read when OpenAI
    SDK methods are called.
    """
    pass  # Disabled - using global context approach instead
...
def setup_observability(config: Optional[ObservabilityConfig] = None) -> None:
    ...
    # Add debug logging patch for OpenAI client
    _patch_openai_client_for_context()
